...
- `/novo-form-web` — use ao criar um formulário protegido novo em `apps/web` (criação/edição de um recurso com rota REST já pronta); padroniza cliente de API tipado, react-hook-form+zod, TanStack Query e tokens visuais — criação usa o padrão de drawer (`components/create-project-drawer.tsx`/`create-library-drawer.tsx` como templates), edição usa o padrão de página (`projects/[id]/edit`/`libraries/[id]/edit`, com `useRequireAuth`)
