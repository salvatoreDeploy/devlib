...
- Criação de projeto em `apps/web` (`/projects/new`): primeira tela protegida do frontend — sem access token válido em `localStorage`, redireciona para `/login` (hook reutilizável `useRequireAuth`). Formulário nome/descrição, chama `POST /projects` com o token e redireciona para `/projects` no sucesso.
- Edição de projeto em `apps/web` (`/projects/[id]/edit`): busca o projeto via `GET /projects/:id`, pré-preenche o formulário, envia `PATCH /projects/:id` no submit e redireciona para `/projects` no sucesso. Mostra erro inline se o projeto não existir/não for do usuário (404) ou se o nome novo colidir com outro projeto (409).
- Dashboard de projetos em `apps/web` (`/projects`): lista os projetos do usuário via `GET /projects` num grid de cards (nome, descrição, data de criação), protegida (`useRequireAuth`). Cada card tem um menu "···" com "Editar" (`/projects/[id]/edit`) e "Excluir" (confirmação via `AlertDialog`, chama `DELETE /projects/:id` e atualiza a lista). O nome do card linka para `/projects/[id]` — tela de detalhe ainda não implementada (próxima subtask do `BACKLOG.md`), então esse link retorna 404 por enquanto. Header compartilhado (`components/header.tsx`) nasce nesta tela: marca "devlib.dev" + e-mail/avatar do usuário logado (decodificado do access token, sem endpoint `/me`); sem tab bar (só aparece dentro de um projeto) e sem menu de logout funcional ainda.
...
- Edição de biblioteca em `apps/web` (`/libraries/[id]/edit`): busca a biblioteca via `GET /libraries/:id`, pré-preenche nome/categoria/notas (mesmo select de categoria de `/libraries/new`), envia `PATCH /libraries/:id` no submit e redireciona para `/` no sucesso. Mostra erro inline se a biblioteca não existir (404) ou se o nome novo colidir com outra (409). Seção "Tags": busca as tags atuais via `GET /libraries/:id/tags` e permite adicionar novas on-the-fly (pill tracejada "+ tag" → input → Enter), chamando `POST /libraries/:id/tags` imediatamente (fora do submit do form) — a pill aparece assim que a API confirma, sem precisar salvar o resto do formulário. Erro de tag duplicada (409) aparece inline, isolado do erro do formulário principal. Sem UI de remoção de tag ainda (API não tem `DELETE`).
...
| Projetos          | `/projects`            | Grid de cards dos projetos do usuário (via `GET /projects`), protegida. Cada card linka pro nome (`/projects/[id]`, 404 até a tela de detalhe existir) e tem menu "···" com Editar/Excluir (`DELETE /projects/:id`, com confirmação). Header compartilhado (marca + usuário logado) aparece aqui pela primeira vez |
| Novo projeto      | `/projects/new`        | Formulário de criação de projeto (nome + descrição), protegida (redireciona para `/login` sem sessão), redireciona para `/projects` no sucesso                                                                                                                                                                     |
| Editar projeto    | `/projects/[id]/edit`  | Formulário de edição de projeto, pré-preenchido via `GET /projects/:id`, protegida, redireciona para `/projects` no sucesso                                                                                                                                                                                        |
...
| Editar biblioteca | `/libraries/[id]/edit` | Formulário de edição de biblioteca, pré-preenchido via `GET /libraries/:id` (nome, categoria e notas), protegida, redireciona para `/` no sucesso. Seção "Tags" (pills + input on-the-fly), fora do submit do form, busca via `GET /libraries/:id/tags` e adiciona via `POST /libraries/:id/tags`                  |
