...
- [ ] Docker: container `web` roda como `root` e quebra `npm run dev` nativo depois — descoberto e documentado em memória de sessão em 2026-09-05/06 (ver `docs/DECISIONS.md`, entradas de 2026-09-06 sobre `.dockerignore`/`docker:clean`). Causa: `apps/web/Dockerfile` não define um `USER` não-root, então qualquer escrita do processo dentro do container (incluindo em `apps/web/.next`, bind-mounted do host) fica com dono `root:root`; se o dev roda `npm run dev` nativo depois de ter usado `docker compose up -d` (web), o Next.js falha com `EACCES: permission denied, unlink '.../.next/server/app-paths-manifest.json'` — não consegue reescrever o próprio cache de build. Contorno atual, sem `sudo`: `docker run --rm -v <repo>/apps/web:/app -w /app alpine rm -rf .next` (repetir toda vez que alternar de Docker pra nativo). Corrigir de vez adicionando um `USER` não-root ao `apps/web/Dockerfile` (criar um usuário com UID/GID igual ao do host via build arg, ou usar o padrão `node` da imagem `node:20-alpine` com permissão de escrita no bind mount) — decidir a abordagem antes de implementar, pode exigir ajuste também no `docker-compose.yml` (`user:`) e não deve quebrar o hot reload existente.
