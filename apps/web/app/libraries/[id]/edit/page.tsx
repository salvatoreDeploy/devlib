...
type EditLibraryFormValues = z.infer<typeof editLibraryFormSchema>;
...
export default function EditLibraryPage() {
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<EditLibraryFormValues>({
    resolver: zodResolver(editLibraryFormSchema),
    defaultValues: { name: "", categoryId: "", notes: "" },
    values: libraryQuery.data
      ? {
          name: libraryQuery.data.name,
          categoryId: libraryQuery.data.categoryId ?? "",
          notes: libraryQuery.data.notes ?? "",
        }
      : undefined,
  });
      ...
      >
        <div>
          ...
          <p className="text-[13px] leading-[1.6] text-muted-foreground">
            Atualize o nome, a categoria ou as notas da biblioteca.
          </p>
        </div>
        ...
        {mutation.isError && (
          <p className="text-xs text-destructive">
            {mutation.error instanceof UpdateLibraryError
